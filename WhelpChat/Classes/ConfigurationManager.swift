...
class ConfigurationManager {
    ...
    func sendApiRequest(configuration: ChatConfiguration, completion: @escaping ((Result<TokenResponse, Error>)->())) {
        let url = URL(string: "https://widget-api.whelp.co/sdk/auth")!
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            else { completion(.failure(WhelpError.notDefinedStatusCode)); return }
            switch statusCode {
            case 200, 201:
                if let data = data {
                    do {
            ...
            case 401:
                if let data = data {
                    do {
                        ...
                        completion(.failure(WhelpError.unauthorized(message: body.message ?? "")))
                    }
                    catch {
                        completion(.failure(error))
                    }
            ...
            default:
                completion(.failure(WhelpError.custom(errorCode: statusCode)))
            }
